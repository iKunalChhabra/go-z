...
func TestDecodeStructCached(t *testing.T) {
	...
	u1, err := zod.DecodeStruct[User](data)
	if err != nil {
		t.Fatalf("DecodeStruct: %v", err)
	}
	u2, err := zod.DecodeStruct[User](data)
	if err != nil {
...
func TestDecodeStructNested(t *testing.T) {
	...
	got, err := zod.DecodeStruct[Person](map[string]any{
		"name": "Lin",
...
func TestDecodeStructJSONTagSkip(t *testing.T) {
	...
	got, err := zod.DecodeStruct[Row](map[string]any{
		"keep": "yes",
		"-":    "no",
		"Skip": "no",
	})
