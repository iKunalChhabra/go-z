...
// LiteralSchema is the Go port of ZodLiteral / $ZodLiteral (Schema[any]).
...
func Literal(values ...any) *LiteralSchema {
	...
	if len(vals) == 0 {
		panic("zod: Literal requires at least one value")
	}
	// Zod also accepts a single array argument as the value list.
	if len(vals) == 1 {
...
func (s *LiteralSchema) Value() any {
	if len(s.values) != 1 {
		panic("zod: LiteralSchema.Value: multiple values; use Values()")
	}
	return s.values[0]
}
