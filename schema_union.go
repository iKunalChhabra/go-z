...
// UnionSchema is the Go port of $ZodUnion / ZodUnion. Output type is any
...
func makeUnionParse(def *Def, options []AnySchemaLike) ParseFn {
	// Single-option fast path: delegate directly (Zod's `first` shortcut).
	if len(options) == 1 {
...
func handleUnionParse(p *Payload, ctx *ParseCtx, def *Def, options []AnySchemaLike) {
	...
	// Zod: if exactly one non-aborted result remains, surface its issues
