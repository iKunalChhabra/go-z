...
// mutate p.Value (overwrite checks). Mirrors $ZodCheck._zod.check.
type CheckFn func(p *Payload)

// Check is the Go port of $ZodCheck: a composable unit of validation attached
...
type Check struct {
	...
	Error ErrorMap
	// Abort stops subsequent checks when this check fails (Zod's {abort:true};
	// issues produced by non-aborting checks carry continue:true).
	Abort bool
	// When gates the check (Zod's `when`): if set, the check runs iff it
...
// runChecks executes a check chain over the payload, porting Zod's runChecks
