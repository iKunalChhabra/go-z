...
// Note: go-zod coerce semantics are intentionally stricter than JS Boolean()/Number()
// for some edge inputs; cases assert actual go-zod behavior.
...
func TestParityCoerceBool(t *testing.T) {
	// go-zod: strict true/false/1/0/"true"/"false"/"1"/"0" — not JS truthiness.
	schema := Coerce.Bool()
	ok := []struct {
		in   any
		want bool
	...
	// JS would coerce these; go-zod rejects.
	fail := []any{"", -1, 3.14, nil, map[string]any{"a": 1}, []any{}}
	for _, in := range fail {
		if schema.SafeParse(in).Success {
			t.Fatalf("Parse(%v) should fail under go-zod coerce", in)
		}
	}
}
...
func TestParityCoerceTime(t *testing.T) {
	// Port: "date coercion" — go-zod accepts RFC3339 strings + unix-ms numbers + time.Time.
	schema := Coerce.Time()
