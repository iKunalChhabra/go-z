...
// IntersectionSchema is the Go port of $ZodIntersection / ZodIntersection.
...
func newIntersection(def *Def, left, right AnySchemaLike) *IntersectionSchema {
	...
	// OptIn/OptOut: either side optional ⇒ intersection optional (Zod).
...
func handleIntersectionResults(result, left, right *Payload, def *Def) {
	...
	// by BOTH sides are re-emitted (Zod's intersection merge of strip/strict).
	...
	if !ok {
		// Zod throws Error; Go prefers a custom issue so SafeParse stays safe.
...
// mergeValues ports Zod's mergeValues: objects merge keys, equal-length arrays
// merge elementwise, identical primitives (incl. equal times) pass through.
// The []any path is the merge-error path (Zod's mergeErrorPath).
func mergeValues(a, b any) (any, []any, bool) {
	// Zod: if (a === b) — reference/primitive equality. Avoid Go == on
...
// sameIntersectionRef reports Zod's === for values safe to compare in Go.
