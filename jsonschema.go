...
// ToJSONSchemaOpts configures ToJSONSchema (subset of Zod's toJSONSchema params).
...
// ToJSONSchema converts a Zod-like schema into a JSON Schema object
...
func emitJSONSchema(schema AnySchemaLike, o ToJSONSchemaOpts, seen map[*Internals]bool) (map[string]any, error) {
	if schema == nil {
		return nil, fmt.Errorf("zod: ToJSONSchema(nil)")
	}
	in := schema.Internals()
	if seen[in] {
...
func handleUnrepresentable(kind string, o ToJSONSchemaOpts) (map[string]any, error) {
	...
	return nil, fmt.Errorf("zod: ToJSONSchema: unrepresentable type %q", kind)
}
...
func applyNumberChecks(out map[string]any, in *Internals) {
	...
	if n, ok := asFloat(in.Bag["exclusiveMinimum"]); ok {
		out["exclusiveMinimum"] = n
	}
	if n, ok := asFloat(in.Bag["exclusiveMaximum"]); ok {
