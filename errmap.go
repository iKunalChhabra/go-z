...
// the chain (Zod's `undefined` return).
type ErrorMap func(iss *Issue) string

// MessageFromString adapts a fixed message to an ErrorMap, mirroring Zod's
...
// Config is the global configuration (Zod's z.config()).
type Config struct {
