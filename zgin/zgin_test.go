...
func userSchema() *z.ObjectSchema {
...
func TestValidateMiddlewareInvalidJSONBody(t *testing.T) {
	...
	if !codes[z.IssueTooSmall] && !codes[z.IssueInvalidFormat] && !codes[z.IssueInvalidType] {
...
func TestBindQueryCoerceNumber(t *testing.T) {
	schema := z.Object(z.Shape{
		"limit":  z.Coerce.Number().Gte(1).Lte(100),
		"offset": z.Default(z.Coerce.Number().Gte(0), float64(0)),
	})
...
func TestBindQueryCoerceNumberInvalid(t *testing.T) {
	...
	found := false
	for _, iss := range resp.Error.Issues {
		if iss.Code == z.IssueInvalidType {
			found = true
			break
		}
	}
...
func TestBindJSONToStruct(t *testing.T) {
	...
	schema := z.ToStruct[User](userSchema())
