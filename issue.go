...
// IssueCode enumerates Zod's issue taxonomy. Values match Zod v4 exactly.
...
// continueMode mirrors Zod's tri-state `continue` flag on raw issues:
...
// Issue is a single validation problem. It doubles as Zod's "raw issue"
// (produced during parsing, message possibly empty) and the finalized issue
// exposed on ZodError; FinalizeIssue converts the former into the latter.
//
// Only fields relevant to the Code are populated. JSON serialization matches
// Zod v4's issue objects.
type Issue struct {
...
// WithContinue marks the issue so subsequent checks still run (Zod's
// `continue: true`, set by non-aborting checks).
func (i Issue) WithContinue() Issue { i.cont = continueYes; return i }

// WithAbort marks the issue as an explicit abort (Zod's `continue: false`).
func (i Issue) WithAbort() Issue { i.cont = continueNo; return i }

// FinalizeIssue resolves the issue message through Zod's precedence chain:
...
// marshalIssues renders issues the way Zod's ZodError.message does:
