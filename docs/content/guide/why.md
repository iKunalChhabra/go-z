...
The TypeScript ecosystem settled on a better model for validating data: schema as value, fluent composition, structured issues, and first-class error maps. go-z brings those **patterns and that architecture** to Go — not a half-hearted “schema-like” surface over struct tags.
...
| Tag validators | Schema-as-value (go-z) |
|---|---|
...
Tags couple validation to a Go type. go-z couples it to a **schema value**, because APIs are JSON-shaped long before they become structs.
...
## Why port architecture (not only the names)

Many schema-style Go libraries copy method names and stop there. go-z ports the **runtime design** that makes composition predictable:
...
That matches `ParsePayload` mental model and unlocks good UX: multiple field errors in one response.
...
### 4. A stable issue taxonomy

Eleven codes, same strings as the upstream library:
...
```text
   ...
   input (any) ───► │ Payload{value, issues}  │
                    ...
                    └───────────┬─────────────┘
                                │
              issues? ──yes──► FinalizeIssue → *Error
                     │
                     no
                     ▼
...
Like TypeScript runtime (which is dynamically typed under `z.infer`), go-z’s engine runs on `any`. Generics appear at the boundary:
...
## When go-z is the right tool

- You want **schema-first muscle memory** in a Go backend
...
- You need compile-time proof that every field of a struct is covered (neither tags nor go-z give you `z.infer` — see [Comparison](#/guide/comparison))

:::info Honest about types
TypeScript libraries can infer static types from schemas (`z.infer<typeof schema>`). Go cannot do that. go-z gives you `Schema[T]` and `ToStruct[T]`, but you still write (or generate) the Go types yourself. The win is runtime composition and issue quality — not deleting type declarations.
:::
...
4. **Parity where it matters** — message and code compatibility with the TypeScript original; Go idioms where Go differs (`error`, generics limits)
5. **Optional integrations** — Gin lives in `zgin`, not the core import graph

Next: [Comparison](#/guide/comparison) for a side-by-side with other Go validators.
