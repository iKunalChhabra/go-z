...
Every issue eventually gets a `Message`. go-z resolves that string through precedence chain: explicit message → producing check/schema map → per-parse map → global custom map → locale → `"Invalid input"`.
...
- [Issues & Error](#/guide/errors) — formatters once you have messages
