...
go-z is built for high-concurrency HTTP workloads: immutable schemas, pooled payloads, precompiled object plans, and a zero-check fast path.
...
After `ReleasePayload`, do not keep references to the payload or its issue slice. Failed parses copy finalized issues into `*z.Error` first.
:::

## Precompiled object plans

At `Object(...)` construction, go-z compiles a stable field plan:
...
Hot-path parse walks `[]objectField` — **no reflection**, no map iteration over the shape. This is the Go analog of JIT `Doc` compilation.
...
Fluent methods that only add checks (`Min`, `Email`, …) clone the def; schemas with **no** checks skip the check loop entirely — a deferred fast path.
