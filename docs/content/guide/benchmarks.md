...
Medians of 3 runs. go-z validates `map[string]any`; validator uses tagged structs; zog parses maps into structs.
...
**Headline:** go-z is **~1.5× faster than validator** sequential and **~3.1× faster than zog**.
...
**Headline:** go-z is **~1.1× faster than validator** and **~2.8× faster than zog**.
...
At the same N, even sequential go-z beats validator’s per-element loop (**4.24 ms** vs **6.09 ms**); parallel widens it to **2.75 ms**. zog takes **12.9 ms**.
...
Format checks beat validator by ~20% since email, UUID, and the ISO date/time formats moved from backtracking regexes to hand-written matchers. Failure-path rendering is still heavier in go-z (schema-shaped finalize + JSON issues).
