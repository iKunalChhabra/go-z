...
Those two rows are not measuring the same job: go-z returns a `map[string]any`,
validator is handed a struct, and zog decodes into one. With `ToStruct` — go-z
ending in a struct too — it is **~1.8× faster than zog** and **~1.2× slower than
validator**. Pick the row that matches what your handler does.
