...
:::info Mental model
Ask two questions for every field: “May it be absent?” and “May it be null?” In TypeScript Zod that’s `.optional()` / `.nullable()` / `.nullish()`. In go-zod it’s the same three wrappers — just remember `Missing` is not `nil`.
:::
