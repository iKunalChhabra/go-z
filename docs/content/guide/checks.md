...
Checks are Zod’s composable validation units. After a schema’s bare type parse succeeds (or at least runs), attached checks inspect — and sometimes mutate — the payload. Fluent helpers like `Min` and `Email` are thin wrappers that attach check objects.
...
- `Abort: true` → issue aborts the remaining chain (like Zod `{ abort: true }`)
...
:::tip Bag metadata
`OnAttach` runs when the check is attached to a schema (construction time). It can record hints like `minimum` / `maximum` / format on `Internals.Bag` — the same idea as Zod’s `inst._zod.bag`.
:::
...
That ports Zod’s subtle `when` behavior: gated checks can still refine after soft failures, but a hard abort wins.
