...
`z.Coerce` mirrors `z.coerce` namespace. Each entry returns the corresponding primitive schema with `Def.Coerce = true`.
...
Almost anything becomes a string via Go formatting (matching JS `String(value)` for common primitives):
...
querySchema := z.Object(z.Shape{
    "page":   z.Int(z.Params{Coerce: true}).Gte(1),
    "limit":  z.Int(z.Params{Coerce: true}).Gte(1).Lte(100),
...
// page/limit are int; active is bool
```
...
form := z.Object(z.Shape{
    ...
    "age":      z.Int(z.Params{Coerce: true}).Gte(0).Lt(150),
...
```go
// With github.com/iKunalChhabra/go-z/zgin — see Integrations → Gin.
// BindQuery coerces url.Values and validates in one step.
r.GET("/search", func(c *gin.Context) {
    query, ok := zgin.BindQuery(c, z.ToStruct[SearchQuery](querySchema))
    if !ok {
...
`BigInt` supports `Params{Coerce: true}` but is **not** on the `z.Coerce` namespace (`z.coerce` also focuses on string/number/boolean/date):
