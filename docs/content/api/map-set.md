...
Go has no built-in JS `Map` / `Set`. go-zod provides stand-ins that accept common Go containers and mirror Zod’s issue taxonomy (`Origin: "map"` / `"set"`).

## Map

`zod.Map(keySchema, valueSchema)` accepts:
...
```go
schema := zod.Map(zod.String(), zod.String())
...
```go
minTwo := zod.Map(zod.String(), zod.String()).Min(2)
minTwo.MustParse(map[any]any{"a": "b", "c": "d"})
res := minTwo.SafeParse(map[any]any{"a": "b"})
// too_small, Origin: "map"

maxTwo := zod.Map(zod.String(), zod.String()).Max(2)
_ = maxTwo.SafeParse(map[any]any{"a": "b", "c": "d", "e": "f"})

justTwo := zod.Map(zod.String(), zod.String()).Size(2)
justTwo.MustParse(map[any]any{"a": "b", "c": "d"})

_ = zod.Map(zod.String(), zod.String()).NonEmpty().SafeParse(map[any]any{})
```
...
```go
minTwo := zod.Set(zod.String()).Min(2)
minTwo.MustParse([]any{"a", "b"})
res := minTwo.SafeParse([]any{"just_one"})
// too_small, Origin: "set"

maxTwo := zod.Set(zod.String()).Max(2)
_ = maxTwo.SafeParse([]any{"one", "two", "three"})

justTwo := zod.Set(zod.String()).Size(2)
justTwo.MustParse([]any{"a", "b"})

_ = zod.Set(zod.String()).NonEmpty().SafeParse([]any{})
```

### Element errors (no path)

Set element issues are **not** path-prefixed (matches Zod’s `handleSetResult`):
...
_ = zod.Set(zod.String()).SafeParse(map[string]any{"a": 1})

_ = zod.Set(zod.String()).SafeParse("nope")
```
