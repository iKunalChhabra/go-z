...
Optional fields are simply absent from `required`. `Int()` carries the JSON safe-integer bound, which is why `age` gets a `maximum` you did not write — the same thing Zod's `toJSONSchema` does for `z.int()`.
...
:::info Export only
There is no `fromJSONSchema`. Going the other way — generating go-zod schemas from a JSON Schema document — is not implemented.
:::
