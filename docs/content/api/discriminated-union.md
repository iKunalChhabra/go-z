...
Object unions keyed by a shared literal field — `z.discriminatedUnion`.
...
At construction, go-z builds a **discriminator map** from each option’s `PropValues` (literal / enum value sets on the discriminator key):
...
Invalid option lists panic at build time:
