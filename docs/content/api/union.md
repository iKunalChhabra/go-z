...
2. **Single option** → fast path: delegate directly to that schema (`first` shortcut).
...
If every option failed but **exactly one** result is non-aborted (e.g. refine/`continue: true` issues), those issues are surfaced **directly** on the parent — not wrapped in `invalid_union`.

### invalid_union Errors

When multiple options abort (or zero non-aborted remain), go-z emits:
