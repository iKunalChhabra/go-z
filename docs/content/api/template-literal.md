...
`TemplateLiteral` validates strings assembled from literal text and other schemas — Zod's `z.templateLiteral([...])`. Parts are concatenated into a single anchored pattern at construction time, so parsing is one regex match.
