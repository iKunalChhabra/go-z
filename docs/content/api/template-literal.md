...
:::warn A numeric part matches digits, not a range
`z.Int32()` contributes an integer *shape*, so a template accepts any run of
digits in that position — including one no `int32` could hold. The width checks
run when you validate the field with its own schema; a single pattern cannot
express them.

```go
id := z.TemplateLiteral([]any{"n", z.Int32()})
id.MustParse("n99999999999999999999") // matches: it is a run of digits
```

Use a template for the shape and validate the number separately when the range
matters.
:::
