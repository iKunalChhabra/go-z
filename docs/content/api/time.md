...
`z.Time()` ports `z.date`. The Go type is `time.Time` (not a calendar-date string — for those, see [`ISODate` / `ISODateTime`](/api/string-formats)).
...
Inclusive bounds. Failed issues use **`Origin: "date"`** and store bounds as **Unix milliseconds** (`Date.getTime`).
