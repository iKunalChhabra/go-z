...
:::info Not a Zod API
TypeScript Zod has no `xor`. It is a go-zod addition for contracts where "matches more than one shape" is a bug. Everything else about it — issue codes, error maps, wrappers — behaves like `Union`.
:::
