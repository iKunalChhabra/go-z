...
// isEmail ports practical email regex:
//
...
// escapes. pattern spells the max UUID in lowercase only, so
...
// isUUID ports uuid regex: any version 1-8 with a valid variant, plus
...
// isGUID ports guid regex: the 8-4-4-4-12 hex layout with no version or
// variant constraint.
func isGUID(s string) bool { return uuidShape(s) }

// isISODate ports date regex, whose alternations encode the Gregorian
...
// isISOTimeDefault ports time regex at the default precision:
...
// isISODateTimeDefault ports datetime regex at its default settings
...
// isIPv4 ports ipv4 regex: four dot-separated decimal octets, each
