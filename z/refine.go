...
// continue subsequent checks (superRefine default). Use Issue.WithAbort()
...
// AddMessage is a string shorthand for AddIssue (ctx.addIssue("msg")).
...
// produce a custom issue. Defaults to continue (non-abort) like refine.
...
// Ports schema-level .overwrite() / CheckOverwrite for AnySchemaLike.
...
// Defaults to abort:true like z.custom().
...
func normalizeRefineParams(params []any) refineParamResult {
	...
	for _, p := range params {
		...
		default:
			panic(fmt.Sprintf("go-z: unsupported refine params type %T (want string, ErrorMap, Params, RefineOpts, or nil)", p))
		}
	}
	return out
}
