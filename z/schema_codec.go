...
func Codec(in, out AnySchemaLike, tx CodecTx) *CodecSchema {
	if in == nil || out == nil {
		panic("go-z: Codec requires non-nil input and output schemas")
	}
	if tx.Decode == nil {
...
// Check attaches raw checks after codec parse (.check / .refine on codecs).
...
func runDirectional(schema AnySchemaLike, data any, dir ParseDirection) (any, error) {
	if schema == nil {
		return nil, fmt.Errorf("go-z: nil schema")
	}
...
// stringifies on encode (jsonCodec helper pattern).
