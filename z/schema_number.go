...
// NumberSchema is the Go port of Number / Number (Schema[float64]).
...
// and normalizes to float64. Rejects NaN and ±Inf (v4).
...
// coerceToNumber ports limited number coercion: strings via ParseFloat,
...
// Step is an alias of MultipleOf (deprecated in).
...
// Safe is identical to Int in v4.
func (s *NumberSchema) Safe(params ...any) *NumberSchema { return s.Int(params...) }

// Finite is a no-op in v4 (numbers already reject Inf).
...
// BigIntSchema is the Go port of BigInt (Schema[*big.Int]).
