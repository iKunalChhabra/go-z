...
// Params customizes a schema or check at construction time, mirroring the
// upstream params union (`string | { error, abort, coerce }`). Fluent methods accept
...
func normalizeParams(params []any) Params {
	...
	for _, raw := range params {
		...
		case paramsSource:
			...
			mergeParams(&out, &src)
		default:
			panic(fmt.Sprintf("go-z: unsupported params type %T (want string, ErrorMap, Params, format opts, or nil)", p))
		}
	}
	return out
}
