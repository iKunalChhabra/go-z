...
// ParseAny validates data with a type-erased schema and returns the untyped
// result. It is the AnySchemaLike counterpart of Schema[T].Parse, for callers
// holding a schema whose output type is not known statically — a middleware, a
// registry, a plugin.
//
//	out, err := z.ParseAny(schema, body)
func ParseAny(schema AnySchemaLike, data any) (any, error) {
	if schema == nil {
		return data, nil
	}
	return parseTyped[any](schema.Internals(), data, nil)
}

// ParseAnyCtx is ParseAny with per-call options.
func ParseAnyCtx(schema AnySchemaLike, data any, ctx *ParseCtx) (any, error) {
	if schema == nil {
		return data, nil
	}
	return parseTyped[any](schema.Internals(), data, ctx)
}
